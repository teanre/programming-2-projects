...
void edit_phone_number(Student& s)
{
    std::string phone_number = "";
                  ...
                  std::map< std::string, Student* > user_ids)
{
    ...
    std::ofstream outputf(file_name);

    // käydään user ids mappi läpi ja tallennetaan studentin arvot
    for (auto pair : user_ids)
    {
        Student s = *(pair.second);
        outputf << s.student_number << ";"
                << s.user_id << ";"
                << s.name << ";"
                << s.phone_number << ";"
                << s.email << ";";
    ...
    while(true) {
        ...
        if(command == "U" or command == "u") {
            ...
            // TODO: Add functionality here
                edit_phone_number(*(student_numbers.at(parts.at(1))));
                save_to_file(file_name, user_ids);
            }
