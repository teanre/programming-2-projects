...
const string PARAMETER_ERROR_MSG = "Error: wrong number of parameters.";
const string OPERAND_ERROR_MSG = "Error: a non-number operand.";
const string UNKNOWN_COMMAND_ERROR_MSG = "Error: unknown command.";
...
int main() {
    ...
    while ( true ) {
        ...
        //convert the command to upper case (if applicable)
        convert_to_upper(command_to_be_executed);

        uint amount_of_parametres = pieces.size() - 1;
        set <string> just_commands;

        // add the commands to a set so it can be checked
        for (auto& s : COMMANDS)
        {
            just_commands.insert(s.str);
        }

        for (auto& s : COMMANDS)
        {
            if (just_commands.find(command_to_be_executed) == just_commands.end())
            {
                cout << UNKNOWN_COMMAND_ERROR_MSG << endl;
                break;
            }
            else
            {
